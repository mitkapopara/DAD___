...
contract DADInterlinkedNodes {
    ...
    event NodeContentUpdated(uint256 nodeId, string newData);
    ...
    function updateNodeContent(uint256 nodeId, string memory newData) external {
        require(
            dataNodes[nodeId].owner == msg.sender,
            "Not the owner of the data node."
        );

        dataNodes[nodeId].data = newData;

        emit NodeContentUpdated(nodeId, newData);
    }
}
